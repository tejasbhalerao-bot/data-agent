"""Cascade levels v2 — Doctor = CALL (not confirmation), two cohorts.

Change vs v1: Doctor leg = actual_doctor_call_time vs digitised_dr_promise (the call, +/-1 min).
Other legs unchanged: WH +/-1 min; Dispatch & Delivery date-level.

Cohorts (both: delivery attempt present, digitised_ts >= 2026-05-08, all 4 legs' timestamps
present, wh_processing_mins != 0):
    ALL      — every order
    COURIER  — digitised_is_sdd == false

Emits 8 Sheets-ready CSVs: cascade-{all,courier}-step{0,1,2,3}-v2.csv
Separate count + percent columns; percents are conditional within parent.
"""
...
def write_cohort(co: Cohort, tag: str) -> None:
    ...
    with open(OUT / f"cascade-{tag}-step2-v2.csv", "w", newline="") as f:
        wr = csv.writer(f)
    ...
    with open(OUT / f"cascade-{tag}-step3-v2.csv", "w", newline="") as f:
        wr = csv.writer(f)
...
def main() -> None:
    ...
    with open(RAW_PATH, newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            ...
            if (row["digitised_is_sdd"] or "").strip().lower() == "false":
    ...
    print("wrote 8 CSVs: cascade-{all,courier}-step{0..3}-v2.csv")
