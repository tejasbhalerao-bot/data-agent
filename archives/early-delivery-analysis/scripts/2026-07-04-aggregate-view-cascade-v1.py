"""View 1 (customer-experienced) and View 2 (ops-execution) leg cascades.

Doctor / Warehouse / Dispatch classified Early/On-Time/Late, cross-tabbed against a
Delivery outcome. Each view sliced 5 ways: ALL (uncut) + 4-way cross of
digitised_is_sdd x digitised_is_inventory (both read at digitise time).
...
Emits per (view, cut): a cascade summary CSV (outputs/viewN-cascade-<cut>.csv) and an
order-level detail CSV with all raw columns + computed leg labels
...
class Cohort:
    def __init__(self) -> None:
        self.tree: dict[tuple[str, str, str], dict[str, int]] = defaultdict(lambda: defaultdict(int))
        self.n = 0

    def add(self, doctor: str, warehouse: str, dispatch: str, delivery: str) -> None:
        self.n += 1
        self.tree[(doctor, warehouse, dispatch)][delivery] += 1


def write_cascade(cohorts: dict[str, Cohort], view_tag: str) -> None:
    for cut_name, co in cohorts.items():
        path = OUT / f"{view_tag}-cascade-{cut_name}.csv"
        with path.open("w", newline="") as f:
            wr = csv.writer(f)
            wr.writerow(
                ["doctor", "warehouse", "dispatch", "count"]
                + ["delivery_early_n", "delivery_early_pct"]
                + ["delivery_ontime_n", "delivery_ontime_pct"]
                + ["delivery_late_n", "delivery_late_pct"]
            )
            for d in OUTCOMES:
                for w in OUTCOMES:
                    for dp in OUTCOMES:
                        counts = co.tree.get((d, w, dp))
                        if not counts:
                            continue
                        total = sum(counts.values())
                        row = [d, w, dp, total]
                        for o in OUTCOMES:
                            row += [counts[o], pct(counts[o], total)]
                        wr.writerow(row)
...
def main() -> None:
    ...
    cohorts_v1: dict[str, Cohort] = {name: Cohort() for name in CUTS}
    cohorts_v2: dict[str, Cohort] = {name: Cohort() for name in CUTS}

    OUT.mkdir(parents=True, exist_ok=True)
    detail_writers_v1: dict[str, csv.DictWriter] = {}
    detail_writers_v2: dict[str, csv.DictWriter] = {}
    detail_files = []
    ...
    with open(BASE_PATH, newline="") as f:
        ...
        detail_fields_v1 = fieldnames + ["doctor_leg", "warehouse_leg", "dispatch_leg", "delivery_leg"]
        detail_fields_v2 = fieldnames + ["doctor_leg", "warehouse_leg", "dispatch_leg", "delivery_leg"]

        for cut_name in CUTS:
            fh1 = open(OUT / f"view1-orders-{cut_name}.csv", "w", newline="")
            fh2 = open(OUT / f"view2-orders-{cut_name}.csv", "w", newline="")
            detail_files += [fh1, fh2]
            w1 = csv.DictWriter(fh1, fieldnames=detail_fields_v1)
            w2 = csv.DictWriter(fh2, fieldnames=detail_fields_v2)
            w1.writeheader()
            w2.writeheader()
            detail_writers_v1[cut_name] = w1
            detail_writers_v2[cut_name] = w2
        ...
        for row in reader:
            ...
            if row_is_complete(row, "actual_doctor_call_time"):
                n_v2_complete += 1
                d, w, dp, dl = classify_view2(row)
                cohorts_v2["all"].add(d, w, dp, dl)
                out_row = dict(row)
                out_row.update(doctor_leg=d, warehouse_leg=w, dispatch_leg=dp, delivery_leg=dl)
                detail_writers_v2["all"].writerow(out_row)
                ck = cut_key(row)
                cohorts_v2[ck].add(d, w, dp, dl)
                detail_writers_v2[ck].writerow(out_row)

    for fh in detail_files:
        fh.close()

    write_cascade(cohorts_v1, "view1")
    write_cascade(cohorts_v2, "view2")
    ...
    print(f"View 2 final N (actual_doctor_call_time leg): {n_v2_complete} ({n_v2_complete / n_total:.1%})")
    for name in CUTS:
        print(f"  view1 cut={name}: n={cohorts_v1[name].n}")
    for name in CUTS:
        print(f"  view2 cut={name}: n={cohorts_v2[name].n}")
