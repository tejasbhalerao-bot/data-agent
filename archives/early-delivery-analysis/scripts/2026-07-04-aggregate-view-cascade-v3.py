...
v3: applies the v2 per-leg marginal fix to View 2 as well. Both views now emit one
row per leg (Doctor/Warehouse/Dispatch/Delivery), each with its own independent
Early/On-Time/Late split -- no cross-tabbing between legs.
...
def main() -> None:
    ...
    cohorts_v2: dict[str, MarginalCohort] = {name: MarginalCohort() for name in CUTS}
    ...
    with open(BASE_PATH, newline="") as f:
        ...
        for cut_name in CUTS:
            ...
            w2 = csv.DictWriter(fh2, fieldnames=detail_fields)
            w1.writeheader()
            w2.writeheader()
    ...
    write_marginal_cascade(cohorts_v2, "view2")
