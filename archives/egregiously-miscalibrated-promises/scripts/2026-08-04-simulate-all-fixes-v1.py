...
# ── C.1  Doctor SLA fix (call-required Late dispatch, on-time dr_confirm sim) ─
# Mechanic: for late-dispatch call-required orders with late dr_confirm,
#           simulate dispatch_sim = dispatch_promise (on-time),
#           delivery_sim = dispatch_promise + actual_TAT
print("C.1 — Simulation: Doctor SLA fix (call-required Late 1d–2d dispatch)")
print("      For DOCTOR_AND_HA_CALL_REQUIRED Late-dispatch orders with late dr_confirm:")
print("      dispatch_sim = dispatch_promise; delivery_sim = dispatch_promise + actual_TAT")
...
for r in late_disp_inv:
    ...
    if not conf or not wh or conf <= wh:
        continue   # dr_confirm on-time — not affected by this fix
    disp_p = to_date(r.get('digitised_dispatch_promise',''))
    da  = to_date(r.get('delivery_attempt_time',''))
    dlp = to_date(r.get('digitised_delivery_promise',''))
    at  = actual_tat(r)
    if not disp_p or not da or not dlp or at is None:
        continue
    n_scope_C1 += 1
    cur_dev = del_dev(r)
    base_C1.append(cur_dev)
    # simulate: pickup = dispatch_promise (on-time), delivery = dispatch_promise + actual TAT
    delivery_sim = disp_p + timedelta(days=at)
    sim_C1.append((dlp - delivery_sim).days)

print(f"  Scope: {n_scope_C1:,} late-dispatch call-required orders with late dr_confirm")
print_sim("Doctor SLA fix", base_C1, sim_C1, n_all_egregious)


# ── C.2  HA call SLA fix (Late 3d+ dispatch, HA-involved, same-day SLA sim) ───
print("C.2 — Simulation: HA call SLA fix (Late 3d+ dispatch, HA-involved orders)")
print("      If HA call had same-day SLA → dispatch_sim = dispatch_promise")
print("      delivery_sim = dispatch_promise + actual_TAT")
...
for r in late_3plus_inv:
    ...
    delivery_sim = disp_p + timedelta(days=at)
    sim_C2.append((dlp - delivery_sim).days)

print(f"  Scope: {n_scope_C2:,} Late 3d+ HA-involved orders")
