...
Determine whether PBA should be scaled up, tuned, or rolled back. Adherence is the primary success metric. Promise TAT is the guardrail metric. Analysis proceeds from outcome → diagnostic → root cause.
...
- How many cases returned an ineligible or invalid courier partner value?

**Why first:** If a material % of the "PBA cohort" ran on fallback logic or received bad courier data, every delta computed downstream is understated or polluted. This is a data integrity gate.
...
**Methodological note:** Adherence comparisons between PBA and Internal are only cleanly interpretable when PBA courier = Internal courier. When they differ, the actual delivery is executed by the PBA-selected courier — so Internal's adherence is measured against a counterfactual promise on a courier that never fulfilled the order. In those cases, the delta conflates courier selection impact and promise construction impact. Segment all adherence comparisons into same-courier and different-courier cohorts and prioritise findings from the same-courier cohort.
...
- Segment: same-courier cases vs different-courier cases
...
- Filter to same-courier cases for clean comparison; report different-courier cases separately
...
### 2.5 Promise calibration vs actual delivery speed
- **Scope:** Filter to cases where PBA partner = Internal partner. When the same courier fulfills the order, actual TAT is identical across both regimes — the only variable is the promise committed.
- Measure: Actual TAT vs PBA promised TAT — how well is PBA's promise calibrated to what the courier actually delivers?
- Measure: Actual TAT vs Internal promised TAT — same calibration check for internal logic
- Which regime is more accurately calibrated to actual delivery speed?
- If PBA's promise is consistently tighter than actual TAT, that is the calibration gap to fix — not the courier selection
...
- Is PBA systematically under-estimating TAT for couriers it prefers?
- Is PBA over-estimating TAT for couriers it de-prioritises, causing it to unfairly downrank them?
...
| Gap from courier mix shift | Cap or blacklist couriers where PBA recommendation accuracy is below threshold |
| Soft ≠ Hard mismatch is structural | Pin courier selection from soft allocation through to hard allocation |
| PBA model improving week-on-week | Hold at current scale, wait for stabilisation before scale-up decision |
...
**Kill condition:** Define minimum adherence floor and sample size threshold before next review. Scale-up is blocked until root cause is resolved.
