...
| `queries-dump/2026-05-25-pba-allocation-with-pref-array-v2.sql` | 4.0 | Same as v1 + AVERAGE_TAT score per dp slot + delivery_partner_id per dp slot (resolved via logistics_delivery_partner.ankw_account_code). Enables Check 1 (allocated == rank-1) in Python script. | Run in Metabase and drop CSV to raw-data/ before running 4.0 script. v1 preserved for reference. |
| `scripts/2026-05-25-aggregate-4-0-ranking-logic-audit-v1.py` | 4.0 | check_id × check_name × total_eligible + pass_count + fail_count + pass_rate_pct. 4 checks: allocated==rank1, EDD ordering, pricing tie-break, AVERAGE_TAT tie-break. Flag any check < 95%. | Reads pba-allocation-with-pref-array CSV (v2). Sample failing rows printed per check. |
| `scripts/2026-05-25-aggregate-4-1-rank-vs-actual-tat-v1.py` | 4.1 | internal_rank_in_pba_array (1–6 / Not In Array) × order_count + delivered_order_count + avg_actual_tat_days + adherence_rate_pct. Monotonicity check printed to console. | Same-courier cohort = rank 1. Different-courier cohort = rank from diff-courier-internal-rank extract. Reads base extract + diff-courier-internal-rank CSV. No new query. |
