-- Base Query 2: Full Clickpost Preference Array per Order
-- One row per order per courier in Clickpost's ranked list.
-- Includes internal courier rank position + TAT comparison.
...
final AS (
    SELECT
        r.*,
        -- Only meaningful when selected_source IN ('PBA_CACHE','PBA_API')
        -- Will be NULL for all rows in shadow mode (selected_source = 'INTERNAL')
        CASE
