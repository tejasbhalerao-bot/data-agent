#!/usr/bin/env python3
"""
1.3 v2 — Same-courier cohort: top-level adherence PBA vs Internal (with optional pre/post cutoff filter)
=========================================================================================================
Filters to same-courier cohort (hard_pba_partner_id == hard_internal_partner_id),
then computes top-level adherence split. Adds --cohort / --cutoff-date for 1.7 support.

Input:  raw-data/*pba-adherence-base-extract*.csv  (latest match)
Output: outputs/2026-05-25-1-3-same-courier-adherence-v2[_PRE|_POST].csv

Usage:
  python 2026-05-25-aggregate-same-courier-adherence-v2.py
  python 2026-05-25-aggregate-same-courier-adherence-v2.py --cohort PRE --cutoff-date 2026-05-20
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='1.3 v2 Same-courier cohort: top-level adherence')
...
def main() -> None:
    ...
    output_path = os.path.join(PROJECT_DIR, 'outputs', f'2026-05-25-1-3-same-courier-adherence-v2{suffix}.csv')
    ...
    print(f"Same-courier cohort: {cohort_size:,} orders ({round(cohort_size * 100.0 / len(df), 2)}% of dataset)")
    ...
    print(f"Excluded (actual_tat <= 0): {excluded_count:,} ({round(excluded_count * 100.0 / cohort_size, 2)}% of cohort)")
    ...
    excluded_row = (same['pba_bucket'] == 'Excluded').sum()
    rows.append({
